...
- **Atlas interpretation:** bubble positions are packing geometry, not network distance; labels distinguish community IDs from member counts. The interface explains which groups actually have subgroups. Internal hidden steps use stationary pulses rather than circular trajectories, displayed-link steps follow the actual SVG paths, and teleportation fades are purple. Opening a group restarts the simulated visitors in that view.
- **Seed sensitivity:** the displayed seed-42 partition is compared with seeds 1, 2, 3, and 99 under the same settings. Agreement is the Jaccard similarity of unordered same-community pairs, excluding isolates. For a character, the comparison shows companions retained/lost/gained; for a reference group, it uses the alternative group with maximum member Jaccard (not a one-to-one group matching). Subgroup views compare their parent top-level group. This is an exploratory sensitivity check, not a confidence estimate.
